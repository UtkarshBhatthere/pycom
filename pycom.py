...
class pycom:
    ...
    project_name    = str()
    out_path        = "Output"
    csv_out_path    = "Output/CSV_Output"
    graph_out_path  = "Output/Graph_Output"
    zip_out_path    = "Output/ZIP_Output"
    out_file_list   = list()    #To store the name of all the files generated by pycom.
    ...
    def output(self, property = "TARC", save = True):
        ...
        # Creates required folder if not already present.
        archiver.make_path(self.csv_out_path)
        archiver.make_path(self.graph_out_path)
        archiver.make_path(self.zip_out_path)
        
        for label in op_values:
            archiver.make_path(f"{self.csv_out_path}/{self.project_name}")
            filename = f"{property}_{label['label']}.csv"
            self.out_file_list.append(filename)
            freq = numpy.array([real(x) for x in label['x']])
            mag  = numpy.array([real(y) for y in label['y']])
            dataFrame = pd.DataFrame({"freq(in Ghz)" : freq, f"{property}_{label['label']}" : mag})
            dataFrame.to_csv(f"{self.csv_out_path}/{self.project_name}/{filename}", index=False)
        ...
        if save == False:
            ...
            archiver.make_path(f"{self.graph_out_path}/{self.project_name}")
            savefile_path = f"{self.graph_out_path}/{self.project_name}/{property}.png"
